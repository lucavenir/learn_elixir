defmodule NewPassport do
  def get_new_passport(now, birthday, form) do
    with {:ok, timestamp} <- enter_building(now),
         {:ok, manual} <- handle_counter_information(now),
  ...
  defp handle_counter_information(%NaiveDateTime{} = datetime) do
    case find_counter_information(datetime) do
      {:coffee_break, _} -> {:retry, NaiveDateTime.add(datetime, 15 * 60, :second)}
